...
| Rule | Detail |
|---|---|
| Persistence | One opaque JSON string under `app_settings["keyboard.keymap"]`. Go never parses it — `internal/settings/store.go`'s `Keymap`/`SetKeymap` delegate straight to `Get`/`Set`. `normalizeChord` (this package) is the sole authority on chord grammar. The keymap also travels in backup bundles as the `keyboard_keymap` group (`internal/settings/backup_export.go`, `backup_import.go`) — a 0-or-1-record group, present-but-empty resets to defaults, an absent group leaves it untouched, and a restored keymap reaches the running dispatcher without a restart (`loadKeymapOverrides`). Its declared footprint (`app_settings` where `key='keyboard.keymap'`, nothing else) is enforced by `internal/desktop/app_backup_import_footprint_test.go`'s guard; see `docs/adr/021-portable-keymap-and-importer-footprints.md` |
