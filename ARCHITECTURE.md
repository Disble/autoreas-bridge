...
* **Source adapters:** each adapter folder under `frontend/src/infrastructure/` is reached by concrete path — `<adapter>/<adapter>.helpers.ts` and its sibling role files — with declaration-placement and schema-colocation enforcement applied across them. There is no `index.ts` entrypoint: ADR-011 removed every barrel in July 2026, this file went on describing one until 2026-08-13, and `frontend/scripts/__tests__/architecture-docs-and-artifacts.test.mjs` was asserting the stale wording. No flat `*-source.ts` compatibility facades remain once an infrastructure adapter is migrated.
