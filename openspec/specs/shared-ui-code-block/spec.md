...
> **Drift note — recorded 2026-08-30 by SDD-65 Slice 0.** The `index.ts` re-export barrel
> required below was removed by `5646bed` ("refactor(frontend): remove barrel imports",
> 2026-08-03) under `docs/adr/011-no-barrel-files.md`, which bans barrel files project-wide.
> `frontend/src/shared/ui/CodeBlock/` today has no `index.ts`; consumers import
> `CodeBlock.tsx` and the colocated `code-block.types.ts` by concrete path. The rest of this
> requirement (no `useEffect`/infrastructure/parsing in the `.tsx`, JSDoc'd pure helpers in
> `code-block.helpers.ts`, state and timers in `use-code-block.ts`, all props `readonly`)
> still holds.
