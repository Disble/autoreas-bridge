...
strings. This covers the `chapter_service*` file family and its `episode_service*`
rename, exported contracts (`ChapterScheduleItem` -> `EpisodeScheduleItem`,
`ChapterDayCount` -> `EpisodeDayCount`, `ChapterCommandResult` -> `EpisodeCommandResult`,
`ListChapterSchedule` -> `ListEpisodeSchedule`, `AdjustWatchedChapters` -> `AdjustWatchedEpisodes`,
`ListChapterDayCounts` -> `ListEpisodeDayCounts`), wiring helpers
(`wireChapterService*` -> `wireEpisodeService*`, `a.chapterService` -> `a.episodeService`),
the `remainingChapters` -> `remainingEpisodes` helper, and the 8 Wails-bound
...
`features/chapters` feature folder renamed to `features/episodes` (including
`ChapterSchedulePanel` -> `EpisodeSchedulePanel`), the `/chapters` route
renamed to `/episodes`, the "Chapters" nav label renamed to "Episodes", the
regenerated `frontend/wailsjs` bindings and their hand-written shims, the
season-store/season-source field `availableChapters` renamed to
`availableEpisodes`, and the fallow dead-code/config ledgers
(`frontend/fallow-list.json`, `fallow-dead-code.json`) that hardcode
`features/chapters/**` paths.
...
behavior using "episode" vocabulary after this change is archived. Archived
...
- **GIVEN** `openspec/specs/anime-editor/spec.md` after this change is archived
