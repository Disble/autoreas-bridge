...
3. **Validation happens on press, in this order** (matching explore §6.3): (a) does this `actionID` belong to *this* `notificationID`; (b) is the intent key registered in the `IntentRegistry` — an unregistered key is **refused**, never resolved by name, shell, or URL; (c) the bound handler validates its own frozen args against live state.
