...
- [ ] 3.3 MUTATE: `ditto staged --exclude-prefix frontend/ --threshold 0.80 --test-command "go test -count=1 -json ."`. **Measured: ~6s per mutant** on the root package; two additive fields make this the smallest Go mutant surface here, so expect well under a minute. Then regenerate `frontend/wailsjs/go/main/App.*`.
