...
- [ ] 1.4 MUTATE 1.2–1.3: `ditto staged --exclude-prefix frontend/ --threshold 0.80 --test-command "go test -count=1 -json ."` (preview with `--dry` first). **Measured: ~6s per mutant** on the root package (4.5s test / 6.2s wall), so ~2 minutes for twenty mutants. ditto is silent between `Releasing Ditto…` and its report — a multi-minute silent run is expected here, not a hang.
...
- [ ] 1.8 RED+GREEN: `SummarizeRuntimeEvents` returns `ByDomain`/`ByLevel`/`ByEventType`/`Samples` from ONE `Reader.Summary` call (D-2 — A2a consumes `ByDomain` at task 2.3, Slice C the rest), honouring `Available`/`Degraded`. [REQ: Runtime Event Summary Surface]
...
- [ ] 1.11 MUTATE 1.5–1.10: `ditto staged --exclude-prefix frontend/ --threshold 0.80 --test-command "go test -count=1 -json ."`. **Measured: ~6s per mutant** on the root package; this is the largest mutant surface in the change (six RED tasks staged), so budget several minutes of silent runtime and confirm the scope with `--dry` before suspecting a hang.
