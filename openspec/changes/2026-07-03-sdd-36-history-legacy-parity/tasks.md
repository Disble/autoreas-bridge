...
- [ ] Scaffold `bun --cwd=frontend run generate:feature history HistoryTable`.
- [ ] RED: `history-table.helpers` tests — long date ("June 30, 2026"), weekday ("Tuesday"),
...
- [ ] GREEN: helpers with JSDoc; page size 10 + debounce delay in `history-table.constants.ts`.

### Phase 3.2 — Hook + table (spec: "History Table With Pagination, Search, and Filters")
- [ ] RED: `use-history-table` tests — loads via `getAnimeHistory` (single call, NO per-item
...
- [ ] GREEN: `use-history-table.ts` (strict hook anatomy).
- [ ] RED: `HistoryTable.tsx` dumb-render tests — HeroUI table with the 7 columns (#, Name,
      ...
      states).
- [ ] GREEN: `HistoryTable.tsx` + wire into `HistoryRoute`.

### Phase 3.3 — Remove the old list
- [ ] Delete `features/history/ui/HistoryList/` (incl. two-step `loadHistoryEntries` fetch and
