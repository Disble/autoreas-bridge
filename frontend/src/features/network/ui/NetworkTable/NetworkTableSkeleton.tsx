import { NETWORK_TABLE_COLUMNS, type NetworkTableColumn } from './network-table.columns';
import { buildNetworkTableSkeletonRows } from './NetworkTableSkeletonRows';

/** One plain header cell of the runtime-events placeholder table, sized from the shared column source of truth. */
function NetworkTableSkeletonHeaderCell({ column }: { readonly column: NetworkTableColumn }): React.JSX.Element {
  return (
...
/**
 * The runtime-events rail's loading placeholder: a PLAIN HTML `<table>` rendered
 ...
 *
 * The header mirrors the real table's five columns (same labels and width
 * classes, from `NETWORK_TABLE_COLUMNS`) and `aria-busy` keeps the busy
 ...
 */
export function NetworkTableSkeleton(): React.JSX.Element {
  return (
    <table aria-busy="true" aria-label="Runtime events" className="w-full table-fixed" data-testid="network-table-skeleton-table">
      <thead>
        <tr>
          {NETWORK_TABLE_COLUMNS.map((column) => (
            <NetworkTableSkeletonHeaderCell column={column} key={column.label} />
          ))}
        </tr>
      </thead>
      <tbody>{buildNetworkTableSkeletonRows()}</tbody>
    </table>
  );
}
