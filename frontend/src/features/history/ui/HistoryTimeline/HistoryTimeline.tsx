import { Alert, Chip, Header, ListBox, Skeleton, Surface } from '@heroui/react';
...
import { formatEpisodeCount, formatRowTime } from '../../../../shared/watch-history/watch-history.helpers';
...
import {
  HISTORY_TIMELINE_DAY_HEADER_CLASS,
...
export function HistoryTimeline() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div className="grid grid-cols-1 gap-4 md:h-[calc(100vh-15.5rem)] md:min-h-96 md:grid-cols-[minmax(0,1fr)_20.5rem]">
        <Surface
          aria-label={HISTORY_TIMELINE_LABEL}
          className="flex max-h-[32rem] min-h-0 min-w-0 flex-col gap-0.5 overflow-y-auto rounded-2xl p-2 md:max-h-none"
        ...
        >
        {isLoading ? (
          <div aria-labelledby="history-timeline-loading-label" aria-live="polite" className="flex flex-col gap-0.5" role="status">
          ...
          >
            {groups.map((group) => (
              <ListBox.Section key={group.dayKey}>
                <Header className={HISTORY_TIMELINE_DAY_HEADER_CLASS}>
                  <span>{group.heading}</span>
                  <span className="normal-case">{formatEpisodeCount(group.count)}</span>
                </Header>
                {group.entries.map((entry) => (
                  <ListBox.Item className={HISTORY_TIMELINE_ROW_CLASS} id={entry.id} key={entry.id} textValue={entry.animeName}>
                    <span className="flex min-w-0 items-center gap-1.5">
                      <span className="min-w-0 truncate font-semibold text-foreground">{entry.animeName}</span>
                      {entry.statusLabel === undefined || entry.statusColor === undefined ? null : (
                        <Chip className="shrink-0" color={entry.statusColor} size="sm" variant="soft">{entry.statusLabel}</Chip>
                      )}
                      {entry.cycle > 1 ? <Chip className="shrink-0" color="warning" size="sm" variant="soft">Rewatch</Chip> : null}
                    </span>
                    <span className="tabular-nums text-muted">Episode {entry.episode}</span>
                    <span className="tabular-nums text-muted">{formatRowTime(entry.watchedAtMs)}</span>
                  </ListBox.Item>
                ))}
              </ListBox.Section>
            ))}
        ...
        <aside aria-label="Anime inspector" className="flex min-h-0 min-w-0">
          <Surface className="flex min-h-0 flex-1 flex-col overflow-y-auto rounded-2xl p-4.5">
