 ...
 * Wails I/O flows through the season store, which refetches on mount (HeroUI
 * Tabs unmount inactive panels) and after each successful mutation — there is
 * no realtime signal listener in `frontend/src` (`season_changed` is mobile-only).
 */
