...
describe('useNotificationToasts', () => {
  ...
  it('keeps toast mapping concerns in colocated helper modules', () => {
    const hookPath = join(process.cwd(), 'src/hooks/use-notification-toasts/use-notification-toasts.ts');
    const helperPath = join(process.cwd(), 'src/hooks/use-notification-toasts/use-notification-toasts.helpers.ts');
    const sourceText = readFileSync(hookPath, 'utf8');
    const helperText = readFileSync(helperPath, 'utf8');

    expect(sourceText).toContain("from './use-notification-toasts.helpers'");
    expect(sourceText).not.toMatch(/const\s+TOAST_BY_LEVEL\b/);
    expect(helperText).toContain('function renderToastByLevel');
    expect(helperText).toContain('export function renderNotificationToast');
  });
});
