...
// fixedNow returns a clock function fixed at the supplied millisecond value.
...
var canonicalOptionalFieldChangeCases = []struct {
	name   string
	before string
	after  string
}{
...
func TestDiffSnapshotsDetectsCanonicalOptionalFieldChanges(t *testing.T) {
	t.Parallel()
	for _, tt := range canonicalOptionalFieldChangeCases {
		tt := tt
