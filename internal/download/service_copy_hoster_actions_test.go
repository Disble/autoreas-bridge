...
// TestOutcomeActionsCopyHosterGivesEveryHosterLinkItsOwnCopyToken is the artboard's jd_offline row:
// "Copy hoster 1" / "Copy hoster 2", one per link, each bound to the anime row it came from and
// each freezing its own link.
func TestOutcomeActionsCopyHosterGivesEveryHosterLinkItsOwnCopyToken(t *testing.T) {
	t.Parallel()
	...
	actions := buildOutcomeActions(kindJDownloaderOffline, "run-1", outcomes)
...
// TestOutcomeActionsCopyHosterKeepsTheWholeNotificationToken pins that the per-row copy tokens are
...
// TestOutcomeActionsCopyHosterNeverBindsTheRerunToken pins what this builder exists to AVOID. The
...
func TestOutcomeActionsCopyHosterNeverBindsTheRerunToken(t *testing.T) {
	...
	for _, action := range actions {
		if action.Intent == "download.run_anime" {
			t.Fatalf("a hoster-blocked row grew a re-run token %#v, want none", action)
		}
	}
}

// TestOutcomeActionsCopyHosterNumbersHostersAcrossEveryEpisodeOfOneRow pins that the numbering is
...
// TestOutcomeActionsCopyHosterBoundsTheButtonsPerRow pins the canvas rule that a notification is not
...
func TestOutcomeActionsCopyHosterBoundsTheButtonsPerRow(t *testing.T) {
	t.Parallel()
	...
	actions := buildOutcomeActions(kindRunStoppedEarly, "run-1", []animeRunOutcome{{animeID: "anime-7", manualLinks: []ManualLink{{Episode: 1, Links: links}}}})
...
// TestOutcomeActionsCopyHosterSkipsWhatCannotBeAddressed pins the two guards. An outcome with no
...
// TestOutcomeActionsCopyHosterFreezesArgsPerToken pins that no two copy tokens share one Args map. A
