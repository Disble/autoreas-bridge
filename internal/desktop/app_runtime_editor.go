...
// GetAnimeEditorRecord loads one anime's full editable record. Unavailability
// and query failures come back as an error outcome inside the result, not as a
// Go error: the binding's single return is what the editor screen renders.
...
// SaveAnimeEditor applies an editor patch. The DTO is converted to the domain
// command first, so a malformed patch fails before touching the write service.
...
// DeactivateAnime takes an anime out of the active rotation, under
// baseModifiedAt optimistic concurrency.
...
// GetAnimeEditorScheduleBoard loads the day-column board the editor drags
// against. originAnimeID marks the card the board was opened from.
...
// ApplyAnimeEditorSchedule commits a dragged schedule draft. The whole draft is
// applied as one command so a partially moved board cannot be persisted.
