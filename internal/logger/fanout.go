...
// EntrySink is the exported form of the package-private entryWriter interface:
...
func NewFanoutLoggerWithSinks(loggers []Logger, sinks ...EntrySink) *FanoutLogger {
	targets := make([]entryWriter, 0, len(loggers)+len(sinks))
	for _, target := range loggers {
		if sink, ok := target.(entryWriter); ok && sink != nil {
