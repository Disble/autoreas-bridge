...
func TestSummaryToolReadOnly(t *testing.T) {
	...
	result, err := summaryRequests(context.Background(), reader, SummaryRequestsInput{Route: "/api/animes/anime-1"})
	if err != nil {
...
func TestSearchFiltersPassthrough(t *testing.T) {
	...
	status := 400
	_, err := searchRequests(context.Background(), reader, SearchRequestsInput{Route: "/api/sync/reconcile", Status: &status, ErrorCode: "apply_pending_failed"})
	if err != nil {
...
func TestSearchDefaultLimit(t *testing.T) {
	...
	result, err := searchRequests(context.Background(), stubToolReader{page: obs.SearchPage{AppliedLimit: 25}}, SearchRequestsInput{})
	if err != nil {
		t.Fatalf("search mobile requests: %v", err)
	}
...
func TestSearchBoundedLimit(t *testing.T) {
	...
	result, err := searchRequests(context.Background(), reader, SearchRequestsInput{Limit: 999, Cursor: "opaque"})
	if err != nil {
		t.Fatalf("search mobile requests: %v", err)
	}
...
func TestSearchMapsInvalidCursor(t *testing.T) {
	...
	_, err = searchRequests(context.Background(), realReader, SearchRequestsInput{Cursor: "not-a-valid-cursor"})
...
func TestGetExposesPersistedReconcileChangelogIDs(t *testing.T) {
	...
	result, err := getRequestContext(context.Background(), &sqliteReader{r: obs.NewReader(db)}, GetRequestContextInput{RequestID: record.RequestID})
	if err != nil {
...
func TestResolveAmbiguousReference(t *testing.T) {
	...
	result, err := resolveRequestContext(context.Background(), stubToolReader{candidates: []ResolveCandidate{{RequestID: "req-1"}, {RequestID: "req-2"}}}, ResolveRequestContextInput{Reference: "phone"})
	if err != nil {
...
func TestGetExactMissNoFallback(t *testing.T) {
	...
	result, err := getRequestContext(context.Background(), stubToolReader{get: obs.GetResult{Found: false}}, GetRequestContextInput{RequestID: "missing"})
	if err != nil {
		t.Fatalf("get mobile request context: %v", err)
	}
	if result.Found {
